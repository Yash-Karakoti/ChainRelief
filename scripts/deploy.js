...
async function main() {
  ...
  const tx = await treasury.createCampaign(
    ...
    hre.ethers.parseEther("50"), // 50 ETH target
    deployer.address
  );
  await tx.wait();
  ...
  if (hre.network.name !== "hardhat" && hre.network.name !== "localhost") {
    ...
    console.log(`npx hardhat verify --network ${hre.network.name} ${treasuryAddress} '${JSON.stringify(admins)}' '${JSON.stringify(approvers)}'`);
    console.log(`npx hardhat verify --network ${hre.network.name} ${distributorAddress} '${JSON.stringify(admins)}'`);
  }
